    ...
    if not self._target_position then
        ...
        elseif self._eventListener then
            ...
            if target_position.y > area.top - 15 then
                target_position.y = area.top - 15 - math.random(50, 100)
            elseif target_position.y < area.bottom + 15 then
                target_position.y = area.bottom + 15 + math.random(50, 100)
            else
                target_position.y = math.random(area.bottom, area.top)
            end
            if target_position.x < area.left + 80 then
                target_position.x = area.left + 80 + math.random(50, 100)
            elseif target_position.x > area.right - 45 then
                target_position.x = area.right - 45 - math.random(50, 100)
            else
                target_position.x = math.random(area.left, area.right)
            end
