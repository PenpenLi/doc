...
function QAIEscape:_execute(args)
    ...
    if d < 0.1 then
        ...
        enemyPos = ccp(enemyPos.x, enemyPos.y) -- 复制，否则会修改actor的成员变量
        if enemyPos.x < CONFIG_SCREEN_WIDTH / 2 then
            enemyPos.x = -10
        else
            enemyPos.x = CONFIG_SCREEN_WIDTH + 10
        end
    ...
    if targetPos.x < BATTLE_AREA.left then
        ...
        targetPos.y = enemyPos.y + math.random(-global.pixel_per_unit * 2, global.pixel_per_unit * 2)
    end
