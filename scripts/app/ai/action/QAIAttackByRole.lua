...
function QAIAttackByRole:_execute(args)
    ...
    local options = self:getOptions()
    local target = self._getTargetEnemy(actor, options.role, options.ranged, options.exclusive)
...
function QAIAttackByRole._getTargetEnemy(actor, role, ranged, exclusive)
    ...
    for i, enemy in ipairs(enemies) do
        ...
        if not enemy:isDead() and math.xor(matched, exclusive) then
