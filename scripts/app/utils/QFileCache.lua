...
function QFileCache:_getFileContent(name, path, cacheTable)
    ...
    if content == nil then
        content = import(app.packageRoot .. "." .. path .. "." .. name)
        if content ~= nil then
...
function QFileCache:getDisplayConfigByName(name)
    ...
    assert(config ~= nil, " display config file named: " .. name .. " does not exist.")
    return config
end
...
function QFileCache:getDisplayClassByName(name)
    ...
    assert(classObj ~= nil, "the display class named: " .. name .. " does not exist.")
    return classObj
end
