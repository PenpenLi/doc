...
QUIDialogGoldBattle.BOOTY_BAY = "activity3_1" --藏宝海湾
QUIDialogGoldBattle.GOLD_TEST = "activity4_1" --黄金试练
...
function QUIDialogGoldBattle:_onTriggerClickFristHandler(tag, menuItem)
	if remote.activityInstance:checkIsOpenForInstanceId(QUIDialogGoldBattle.BOOTY_BAY) == true then
		if self._activity1CD > 0 then
...
function QUIDialogGoldBattle:_onTriggerClickSecondHandler(tag, menuItem)
	if remote.activityInstance:checkIsOpenForInstanceId(QUIDialogGoldBattle.GOLD_TEST) == true then
		if self._activity2CD > 0 then
			app.tip:floatTip(QUIDialogGoldBattle.CD_ERRORTEXT)
			return
		end
