...
function QUIDialogSystemPrompt:ctor(options)
    ...
    if options then
        ...
        elseif options.buyFlag ~= nil then
            ...
            local str = nil
            if good ~= nil then
                str = string.format("是否花费%d购买%d战旗", good.token_cost, good.return_count)
            else
                str = "物品不存在"
            end
            ...
            local s = {"A", "B", "C", "D"}
            local str = string.format("确实选择“赛区 %s”进行这一周的竞技场角逐么？", s[self._options])
            self._ccbOwner.label_line2:setVisible(true) 
            self._ccbOwner.label_line1:setString(str)
            self._ccbOwner.label_line2:setString("选择后直到下周一凌晨4点竞技场刷新前无法变更")
        end
    end
end
