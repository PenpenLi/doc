...
function QSBPlayWarningZone:_execute(dt)
	...
	if self._isExecuting ~= true then
		...
		self._effect = app.scene:displayWarningZone(options.effect_id, position, self._skill:getSectorRadius() * global.pixel_per_unit, duration, cc.c4f(1, 1, 1, 0.2), 1.0, 0.5, 360)
		self._effect:retain()
