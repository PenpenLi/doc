...
function QSBManualMode:_execute(dt)
	...
	if self:getOptions().enter == true then
		if self._attacker:getManualMode() ~= self._attacker.STAY then -- 这里应该把ATTACK模式也转入STAY模式，保证移动不会被打断
        	QSBManualMode.entered_actor[self._attacker] = self._attacker:getManualMode()
        	self._attacker:setManualMode(self._attacker.STAY)
    	end
	elseif self:getOptions().exit == true then
		if self._attacker:getManualMode() == self._attacker.STAY and QSBManualMode.entered_actor[self._attacker] ~= nil then
        	self._attacker:setManualMode(QSBManualMode.entered_actor[self._attacker])
        	QSBManualMode.entered_actor[self._attacker] = nil
