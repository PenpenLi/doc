...
function QEArenaDatabaser:ctor()
	...
	while index <= #self._heroCombines do
		...
		for _, heroId in ipairs(heroIds) do
			...
			local talentInfo = QStaticDatabase:sharedDatabase():getTalentByID(characterInfo.talent)
			if talentInfo.func == "t" then
				t = t + 1
			elseif talentInfo.func == "dps" then
				dps = dps + 1
			elseif talentInfo.func == "health" then
				health = health + 1
			end
		end
...
function QEArenaDatabaser:start()
	...
	if SKIP_BATTLE_PHASE == true and CCFileUtils:sharedFileUtils():isFileExist(self._fileName) == true then
		...
		SKIP_BATTLE_SOUND = true
		app.editor.databaser = self
		self._currentIndex = 0
		CCDirector:sharedDirector():skipDrawAndNoIntervalLoop()
		self:_onStart()
	end
end
