...
local function stack(start, over)
...
local function debug_hook(event, line)
  ...
  elseif event == "line" then
    ...
    local vars, status, res, start, over
    if (watchescnt > 0) then
      ...
      for index, value in pairs(watches) do
        ...
        if ok and fired then
          status, res, start, over = cororesume(coro_debugger, events.WATCH, vars, file, line, index)
    ...
    if getin then
      ...
      step_into = false
      step_over = false
      status, res, start, over = cororesume(coro_debugger, events.BREAK, vars, file, line)
    end
    ...
    if status and res == 'stack' then
      while status and res == 'stack' do
        ...
        local a = ngx and 5 or 4
        local ok, snapshot
        if start then
          if over then
            ok, snapshot = pcall(stack, start + a, over + a)
          else
            ok, snapshot = pcall(stack, start + a)
          end
        else
          ok, snapshot = pcall(stack, a)
        end
        status, res, start, over = cororesume(coro_debugger, ok and events.STACK or events.BREAK, snapshot, file, line)
      end
    end
...
local function debugger_loop(sev, svars, sfile, sline)
  ...
  while true do
    ...
    elseif command == "STACK" then
      ...
      local vars, ev = {}
      local _, _, _, start, over = string.find(line, "^([A-Z]+)%s+(.-)%s+(%d+)%s*$")
      print("start , over", start, over, line)
      start = tonumber(start)
      over = tonumber(over)
      if seen_hook then
        ev, vars = coroyield("stack", start, over)
      end
...
local function handle(params, client, options)
  ...
  elseif command == "suspend" then
    client:send("SUSPEND\n")
  elseif command == "stack" then
    local _, _, start_over = string.find(params, "(%d+)")
    local start, over
    if start_over then
      start_over = tonumber(start_over)
      start = math.floor(start_over / 10000)
      over = start_over - 10000 * start
    end
    if start and start ~= "nil" then
      if over and over ~= "nil" then
        client:send("STACK " .. start .. " " .. over .. "\n")
      else
        client:send("STACK " .. start .. "\n")
      end
    else
      client:send("STACK\n")
    end
    -- client:send("STACK 2 2\n")
